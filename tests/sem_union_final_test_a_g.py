...
from lotus.sem_ops.sem_union_large import sem_union
...
user_instruction = (
            ...
            f"Row 1 columns: {{'title', 'description', 'manufacturer', 'price'}}\n"
            f"Row 2 columns: {{'title', 'description', 'manufacturer', 'price'}}\n\n"
...
# Save results
result_df.to_csv("tests/amazon_google_sem_union_results.csv", index=False)
...
try:
    ...
    for _, our_row in result_df.iterrows():
        ...
        #our_id = str(our_row['track_id']) if 'track_id' in our_row else ''
        our_id = str(our_row['id']) 
